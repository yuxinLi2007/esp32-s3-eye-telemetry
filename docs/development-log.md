...
## 阶段 17 · 第4周：让大模型当"意图翻译官"

目标只有一句话：用户说人话，系统识别意图并调用已有的两个动作——查看历史数据、
请求一次新采集。不新造数据通道，也不新造状态机；助手是编排层。

### 设计选择：模型只翻译，不执行

让模型直接调接口，等于把"能改设备状态的手"交给一个会编 MAC、编参数的黑盒。
所以模型的唯一产出是结构化意图 JSON（`intent + slots`），设备必须过白名单校验，
真正的动作由服务端调用已有接口完成。回复里的数字也由服务端从库里取出后拼——
第1~3周所有取舍都在拒绝"数字失真"，没道理第4周把这条底线交给模型。

两个动作的时间语义**相反**，这是本周最危险的一处：
"查看"必须保留旧 `t_server_recv_ms`，"采集"必须等设备回执产生新样本。
规则与模型给出不同动作时强制 `clarify`，谁都不许替用户拍板——把"查看"当成"采集"
会白扰动设备，把"采集"当成"查看"会让用户以为拿到了新数据。

### 坑 1：不配 key 也必须能用

最初把模型当必经之路，没 key 时助手整个不可用。这违反前3周"失败必须可见、可用性优先"
的口径，也让断网/离线无法演示。改成双引擎：无 key / 模型报错 / 返回垃圾全部降级到
规则引擎，`trace.llm_error` 留痕。`engine=rules` 还能强制一个网络包都不发，
用于离线验收。

### 坑 2：模型输出必须当不可信输入

`_sanitize_llm()` 只保留白名单里的 `intent` 和三个已知 slots；点名了白名单外的设备，
整条意图改判 `reject/forbidden_device`。而且执行动作前**再校验一次**设备——
安全不能依赖"上游一定守规矩"。测试里把 `parse_llm` 换成"什么都挡不住"的实现，
越权请求依然被拒。

### 坑 3：等待窗口用尽 ≠ 设备超时

"下发指令 → sleep 2 秒 → 查库"会在网络抖动时拿旧样本冒充新样本，正是本周要防的错。
改成轮询指令状态直到终态。等待窗口到了但指令还活着时返回 `device_no_response`
（带 request_id 让调用方继续查），只有状态机真的结算成 `timeout` 才是 `device_timeout`。
谎报超时和拿旧样本充新一样不可接受。

### 验证

- `pytest` 从 130 → **156 项**（新增 26 项，覆盖新旧数据、越界、含糊、无响应、
  模型幻觉/冲突/降级、参数夹取、信封永远 200）。
- `nl_demo.py` 打**真实 HTTP**、由假设备扮演板端领取+回执，**17 通过 / 0 失败**，
  用独立端口 + 独立库，不污染正在用的 `data/telemetry.db`。
- 新增 `docs/week4-assistant.md`：错误码表、接口、前端、两组必测场景的判定要点。

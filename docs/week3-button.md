...
### 4.5.1 真机缺陷：断网时按键不闪灯，本地反馈被网络超时卡死

**现象**（2026-09-22 上板）：服务端停掉后再按 BOOT，灯不闪、事件也不上报；
恢复服务端也没有补传。断网前按是正常的。

**根因不在按键，在网络阻塞主循环。** 原实现里按键是主循环轮询扫描的，
而 `loop()` 同一轮里要发上传/领指令/回执等多个 HTTP 请求。服务端一停，
每个请求都要等满连接超时，实测上传节拍从 2s 变成 **8.02s**（每轮被堵约 6 秒）。
一次普通点按约 100ms，整个落进阻塞窗口 → 扫描根本没看见 → 不闪灯、无事件。

**这是"本地反馈不等网络"这条设计承诺的直接违背**：`led_play()` 确实排在上传
之前，但前提是 `on_press()` 能被调用——而它压根没被调用。

**修法（两处，都是把"本地"和"网络"真正隔开）：**

1. `firmware/src/button.cpp`：按键扫描 + LED 图案移入独立 FreeRTOS 任务
   （`button_task`，优先级 3 > loop 的 1），主循环只保留上行重试。
   LED 只有一个属主（按键任务），主循环要放远程反馈只能登记 `g_pattern_req`；
   待传队列由 `g_q_lock` 互斥量保护，HTTP 调用一律在锁外。
2. `firmware/src/uplink.cpp`：加断网熔断。连续失败后 2s 起退避、翻倍、封顶 8s，
   窗口内直接返回 `UPLINK_NO_WIFI`，不再拿连接超时去堵主循环。

**实测（服务端保持关闭，抓 70 秒串口）：** 上传节拍稳定 **2.00s**（修前 8.02s）；
按 BOOT 三次，`[btn] 按下 seq=0/1/2（LED 已本地反馈）` 逐条出现，并各自打印
"上传失败…事件不会丢"。恢复正常服务端后三条全部补传成功（库中事件 26 → 29）。
